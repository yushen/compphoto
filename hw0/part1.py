...
def interlace(evens, odds):
  ...
  shape = (evens.shape[0]*2, evens.shape[1], evens.shape[2])
  outimg = np.array([0]* reduce(lambda x,y:x*y,shape),dtype="uint8")
  outimg.shape = shape
  outimg[0:evens.shape[0]*2:2,:,:] = evens
  outimg[1:evens.shape[0]*2:2,:,:] = odds
  #---------------------------------------------------------------------------
  return outimg
