...
def convolve(image, kernel):
  ...
  cols = shape[1]
  d = np.shape(kernel)[0]
  output = np.ndarray((rows - d + 1, cols - d + 1))
  for i in np.arange(0, rows - d + 1):
    for j in np.arange(0, cols - d + 1):
      x = 0
      for di in np.arange(0, d):
        for dj in np.arange(0, d):
          x = x + kernel[di,dj] * image[i + d - di - 1, j + d - dj - 1]
      if x > 255:
        x = 255
      elif x < 0:
        x = 0
